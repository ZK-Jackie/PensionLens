    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:index_age_his:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:index_age_his:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:index_age_his:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:index_age_his:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="index_age_hisList" @selection-change="handleSelectionChange">
...
<script>
import { listIndex_age_his, getIndex_age_his, delIndex_age_his, addIndex_age_his, updateIndex_age_his } from "@/api/system/index_age_his";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listIndex_age_his(this.queryParams).then(response => {
        this.index_age_hisList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加测算方案参数历史表（按年龄）";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getIndex_age_his(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改测算方案参数历史表（按年龄）";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateIndex_age_his(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addIndex_age_his(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除测算方案参数历史表（按年龄）编号为"' + ids + '"的数据项？').then(function() {
        return delIndex_age_his(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/index_age_his/export', {
        ...this.queryParams
      }, `index_age_his_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
