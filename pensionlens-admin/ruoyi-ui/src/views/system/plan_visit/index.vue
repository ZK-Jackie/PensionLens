    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:plan_visit:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:plan_visit:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:plan_visit:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:plan_visit:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="plan_visitList" @selection-change="handleSelectionChange">
...
<script>
import { listPlan_visit, getPlan_visit, delPlan_visit, addPlan_visit, updatePlan_visit } from "@/api/system/plan_visit";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPlan_visit(this.queryParams).then(response => {
        this.plan_visitList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加测算方案访问表";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getPlan_visit(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改测算方案访问表";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePlan_visit(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPlan_visit(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除测算方案访问表编号为"' + ids + '"的数据项？').then(function() {
        return delPlan_visit(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/plan_visit/export', {
        ...this.queryParams
      }, `plan_visit_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
