<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="是否是预测大屏，N-否，Y-是" prop="isPredict">
        <el-input
          v-model="queryParams.isPredict"
          placeholder="请输入是否是预测大屏，N-否，Y-是"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="是否启用，N-否，Y-是" prop="isActive">
        <el-input
          v-model="queryParams.isActive"
          placeholder="请输入是否启用，N-否，Y-是"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:screen:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="screenList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="主键id" align="center" prop="screenId" />
      <el-table-column label="大屏名称" align="center" prop="screenName" />
      <el-table-column label="是否是预测大屏，N-否，Y-是" align="center" prop="isPredict" />
      <el-table-column label="是否启用，N-否，Y-是" align="center" prop="isActive" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否是预测大屏，N-否，Y-是" prop="isPredict">
          <el-input v-model="form.isPredict" placeholder="请输入是否是预测大屏，N-否，Y-是" />
        </el-form-item>
        <el-form-item label="是否启用，N-否，Y-是" prop="isActive">
          <el-input v-model="form.isActive" placeholder="请输入是否启用，N-否，Y-是" />
        </el-form-item>
        <el-form-item label="最后更新人id" prop="lastUpdateUserId">
...
<script>
import { listScreen, getScreen, delScreen, addScreen, updateScreen } from "@/api/system/screen";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        isPredict: [
          { required: true, message: "是否是预测大屏，N-否，Y-是不能为空", trigger: "blur" }
        ],
        isActive: [
          { required: true, message: "是否启用，N-否，Y-是不能为空", trigger: "blur" }
        ],
        lastUpdateTime: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除大屏表编号为"' + screenIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/screen/export', {
        ...this.queryParams
