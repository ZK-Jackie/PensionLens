...
<script>
import { listResults, getResults, delResults, addResults, updateResults } from "@/api/system/results";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除结果预测表编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/results/export', {
        ...this.queryParams
