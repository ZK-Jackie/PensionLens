    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:index_age:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:index_age:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:index_age:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:index_age:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="index_ageList" @selection-change="handleSelectionChange">
...
<script>
import { listIndex_age, getIndex_age, delIndex_age, addIndex_age, updateIndex_age } from "@/api/system/index_age";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listIndex_age(this.queryParams).then(response => {
        this.index_ageList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加测算方案参数表（按年龄）";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getIndex_age(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改测算方案参数表（按年龄）";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateIndex_age(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addIndex_age(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除测算方案参数表（按年龄）编号为"' + ids + '"的数据项？').then(function() {
        return delIndex_age(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/index_age/export', {
        ...this.queryParams
      }, `index_age_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
