    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:index_year:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:index_year:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:index_year:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:index_year:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="index_yearList" @selection-change="handleSelectionChange">
...
<script>
import { listIndex_year, getIndex_year, delIndex_year, addIndex_year, updateIndex_year } from "@/api/system/index_year";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listIndex_year(this.queryParams).then(response => {
        this.index_yearList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加测算方案参数表（按年）";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getIndex_year(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改测算方案参数表（按年）";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateIndex_year(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addIndex_year(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除测算方案参数表（按年）编号为"' + ids + '"的数据项？').then(function() {
        return delIndex_year(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/index_year/export', {
        ...this.queryParams
      }, `index_year_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
