    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:index_urban_his:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:index_urban_his:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:index_urban_his:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:index_urban_his:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="index_urban_hisList" @selection-change="handleSelectionChange">
...
<script>
import { listIndex_urban_his, getIndex_urban_his, delIndex_urban_his, addIndex_urban_his, updateIndex_urban_his } from "@/api/system/index_urban_his";

export default {
  name: "Index_urban_his",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listIndex_urban_his(this.queryParams).then(response => {
        this.index_urban_hisList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加测算城镇化率历史表";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getIndex_urban_his(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改测算城镇化率历史表";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateIndex_urban_his(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addIndex_urban_his(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除测算城镇化率历史表编号为"' + ids + '"的数据项？').then(function() {
        return delIndex_urban_his(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/index_urban_his/export', {
        ...this.queryParams
      }, `index_urban_his_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
