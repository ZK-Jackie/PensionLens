...
public class SysUserController {
    ...
    public Result login(@RequestBody UserLoginRequest user){
        if (user == null){
            return Result.error(HttpStatus.BAD_REQUEST,"参数不能为空");
        }
        // 1. 验证码校验
    ...
    public Result validateCaptcha(String username, String code, String uuid){
        ...
        if (captcha == null){
            return Result.error(HttpStatus.BAD_REQUEST,"验证码已失效");
        }
        if (!code.equalsIgnoreCase(captcha)){
            return Result.error(HttpStatus.BAD_REQUEST,"验证码错误");
        }
        return null;
    }
    ...
    public Result loginPreCheck(String username, String password){
        if(username == null || password == null){
            return Result.error(HttpStatus.BAD_REQUEST,"用户名或密码不能为空");
        }
